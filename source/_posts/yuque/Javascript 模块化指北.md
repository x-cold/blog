...
模块化编程就是通过组合一些**相对独立可复用的模块**定义模块**引入模块**；
...
尽管 JavaScript 语言层面并未提供模块化的解决方案，但利用其可**面向对象**设计模式**加持，能够实现一些简单的模块化的架构；经典的一个案例是利用单例模式模式去实现模块化，可以对模块进行较好的封装，只暴露部分信息给需要使用模块的地方；
